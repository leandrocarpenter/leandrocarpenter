# Leandro Carpenter

<div align="center">
  <img src="https://readme-typing-svg.herokuapp.com/?lines=SRE+Observability;Python+Developer;Node.js+%26+TypeScript&center=true&width=380&height=45">
</div>

## About

SRE focused on **Observability** and **Infrastructure Monitoring**. Dedicated to building reliable, scalable, and observable systems. Experience with **Python**, **Node.js**, and **TypeScript** for modern observability solutions.

## Technical Stack

### Programming Languages
![Python](https://img.shields.io/badge/-Python-3776AB?style=flat-square&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/-JavaScript-F7DF1E?style=flat-square&logo=javascript&logoColor=black)
![TypeScript](https://img.shields.io/badge/-TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![Node.js](https://img.shields.io/badge/-Node.js-339933?style=flat-square&logo=node.js&logoColor=white)

### Observability & Monitoring
![Prometheus](https://img.shields.io/badge/-Prometheus-E6522C?style=flat-square&logo=prometheus&logoColor=white)
![Grafana](https://img.shields.io/badge/-Grafana-F46800?style=flat-square&logo=grafana&logoColor=white)
![Jaeger](https://img.shields.io/badge/-Jaeger-66CFE3?style=flat-square&logo=jaeger&logoColor=white)
![OpenTelemetry](https://img.shields.io/badge/-OpenTelemetry-000000?style=flat-square&logo=opentelemetry&logoColor=white)

### DevOps & Infrastructure
![Kubernetes](https://img.shields.io/badge/-Kubernetes-326CE5?style=flat-square&logo=kubernetes&logoColor=white)
![Docker](https://img.shields.io/badge/-Docker-2496ED?style=flat-square&logo=docker&logoColor=white)
![Kind](https://img.shields.io/badge/-Kind-326CE5?style=flat-square&logo=kubernetes&logoColor=white)
![Helm](https://img.shields.io/badge/-Helm-0F1689?style=flat-square&logo=helm&logoColor=white)

## Featured Projects

### [Observability Stack K8s](https://github.com/leandrocarpenter/observability-stack-k8s)
Complete observability stack for local Kubernetes environments:
- **Kind** - Local Kubernetes cluster management
- **Prometheus** - Metrics collection and storage
- **Grafana** - Data visualization and dashboards
- **Jaeger** - Distributed tracing system
- **Alertmanager** - Alert handling and routing
- **Node.js sample app** - Instrumented demonstration application

### [TypeScript API Observability](https://github.com/leandrocarpenter/typescript-api-observability)
Modern TypeScript REST API with comprehensive observability:
- **TypeScript** + **Express.js** - Type-safe backend development
- **OpenTelemetry** - Automated instrumentation
- **Prometheus metrics** - Custom application metrics
- **Winston** - Structured logging
- **Swagger/OpenAPI** - Interactive API documentation
- **Jest** - Automated testing framework

## GitHub Statistics

<div align="center">
  <img height="180em" src="https://github-readme-stats.vercel.app/api?username=leandrocarpenter&show_icons=true&theme=dark&include_all_commits=true&count_private=true"/>
  <img height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=leandrocarpenter&layout=compact&langs_count=8&theme=dark"/>
</div>

## Current Focus

- **Node.js** - Backend development and API design
- **TypeScript** - Static typing for JavaScript applications
- **Advanced Observability** - OpenTelemetry and distributed tracing
- **GitOps** - ArgoCD and Flux for deployment automation

## Contact

[![LinkedIn](https://img.shields.io/badge/-LinkedIn-0077B5?style=flat-square&logo=linkedin&logoColor=white)](https://linkedin.com/in/leandrocarpenter)
[![Email](https://img.shields.io/badge/-Email-D14836?style=flat-square&logo=gmail&logoColor=white)](mailto:leandro.carpenter@example.com)